...
export function registerSpreadsheetImportRoutes(app: express.Express) {
  app.get("/api/export/spreadsheet", requireRole("planner"), asyncHandler(async (req, res) => {
  ...
  app.post("/api/import/spreadsheet", requireRole("planner"), asyncHandler(async (req, res) => {
    const auth = currentAuth(req) as AuthContext;
    ...
    try {
      ...
      await createUndoSnapshot(client, auth, "Import spreadsheet crop plan");
