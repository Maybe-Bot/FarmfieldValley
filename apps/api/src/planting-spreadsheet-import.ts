...
export function parsePlantingTemplateRows(rows: SpreadsheetRow[]) {
  ...
  for (const row of rows) {
    ...
    if (!crop) {
      throw new Error(`Row ${row.rowIndex}: Crop is required`);
    }
    if (!field) {
      throw new Error(`Row ${row.rowIndex}: Field is required`);
    }
    if (!block) {
      throw new Error(`Row ${row.rowIndex}: Block is required`);
    }
    const startDate = parseIsoDate(startDateRaw, row.rowIndex, "Start date");
    if (!startDate) {
      throw new Error(`Row ${row.rowIndex}: Start date is required`);
    }
    ...
    templateRows.push({
      ...
      catalogNumber,
      startDate,
      plantCount: parseRequiredInteger(plantCountRaw, row.rowIndex, "Plant count"),
      ...
      fieldSpacingInRow: parseOptionalNumber(fieldSpacingRaw, row.rowIndex, "Field spacing in row"),
      rowSpacing: parseOptionalNumber(rowSpacingRaw, row.rowIndex, "Row spacing"),
      rowsPerBed: parseOptionalInteger(rowsPerBedRaw, row.rowIndex, "Rows per bed"),
...
async function loadBedLookup(client: DbClient, farmId: number) {
  ...
  return new Map(result.rows.map((row) => [lookupKey(row.fieldName, row.blockName, row.bedName), row.bedId]));
}

async function ensureFieldBlockExists(client: DbClient, farmId: number, row: ImportRow) {
  ...
  if (!result.rows[0]) {
    throw new Error(`Row ${row.sourceRow}: Field/Block not found: ${row.field} / ${row.block}`);
  }
}
...
async function upsertSeedItem(
  ...
  const existing = await client.query<{ id: number }>(
    `
      ...
      limit 1
    `,
    [farmId, row.crop, row.variety, row.seedSupplier, row.catalogNumber]
  );
  if (existing.rows[0]) {
...
export async function importPlantingSpreadsheetRowsForFarm(client: DbClient, rows: SpreadsheetRow[], farmId: number) {
  ...
  for (const row of parsedRows) {
    await ensureFieldBlockExists(client, farmId, row);
    const intendedBedId = row.bed ? bedLookup.get(lookupKey(row.field, row.block, row.bed)) ?? null : null;
    if (row.bed && intendedBedId == null) {
      throw new Error(`Row ${row.sourceRow}: Bed not found: ${row.field} / ${row.block} / ${row.bed}`);
    }

    const { cropId, varietyId } = await upsertCropAndVariety(client, row.crop, row.variety);
    ...
    const notes = [
      ...
      `Field/Block/Bed: ${[row.field, row.block, row.bed || "unassigned bed"].join(" / ")}.`,
    ...
    const title = `${row.crop}${row.variety ? ` - ${row.variety}` : ""}${row.bed ? ` (${row.bed})` : ""}`;

    const plantingResult = await client.query<{ id: number }>(
      `
        insert into plantings (
          farm_id, crop_id, variety_id, seed_item_id, title, status, intended_bed_id,
